...
asvtab.rare.5000 <- rarefy(asvtab, sample = raremin, se = TRUE)
dim(asvtab.rare.5000)
```
...
asvtab.rare.10500 <- rarefy(asvtab, sample = raremin, se = TRUE)
dim(asvtab.rare.10500)
```

```{r}
# How does this compare to actual number passed to analysis below?
rarefaction.df <- 
     data.frame(threshold = 5000,
                obs_rare = asvtab.rare.5000[1, ],
                obs_rare_se = asvtab.rare.5000[2, ]) %>% 
     rownames_to_column(var = 'name')

rarefaction.df <- 
     data.frame(threshold = 10500,
                obs_rare = asvtab.rare.10500[1, ],
                obs_rare_se = asvtab.rare.10500[2, ]) %>% 
     rownames_to_column(var = 'name') %>% 
     bind_rows(rarefaction.df)

# Pivot wider (one row per sample)
rarefaction.df <- 
     pivot_wider(rarefaction.df,
                 names_from = threshold,
                 names_sep = '_',
                 values_from = c(obs_rare, obs_rare_se),
                 values_fill = NA)
...
ggplot(rarefaction.df, aes(x = obs, y = obs_rare_5000)) + 
     geom_abline(slope = 1, intercept = 0, color = 'red') +
     geom_point() +
     coord_equal() +
     labs(x = 'Observed taxa (raw)', y = 'Observed taxa (rarefied)',
          title = '5,000 read threshold') + 
     theme_bw()
```
```{r}
plotID <- paste(Sys.Date(), as.character(sample(000:999,1)), sep=".")
...
# ggsave(here('results', 'manuscript', 'supplemental',
#             paste0(plotID, '_Observed taxa, 5,000 read rarefaction.pdf')),
#        height = 4, width = 4)
```

```{r}
# Could maybe add standard error on top of this?
ggplot(rarefaction.df, aes(x = obs, y = obs_rare_10500)) + 
     geom_abline(slope = 1, intercept = 0, color = 'red') +
     geom_point() +
     coord_equal() +
     labs(x = 'Observed taxa (raw)', y = 'Observed taxa (rarefied)',
          title = '10,500 read threshold') + 
     theme_bw()
```
```{r}
plotID <- paste(Sys.Date(), as.character(sample(000:999,1)), sep=".")
...
# ggsave(here('results', 'manuscript', 'supplemental',
#             paste0(plotID, '_Observed taxa, 10,500 read rarefaction.pdf')),
#        height = 4, width = 4)
```
