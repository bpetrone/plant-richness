...
samdf %>% 
     ...
     filter(reads_tot >= 1000) %>% 
     mutate(pct_no_id = 1 - (reads_food/reads_tot)) %>% 
     ggplot(aes(x = pct_no_id)) +
     geom_histogram() +
     facet_wrap(~study) +
     labs(y = 'Frequency', x = 'Unidentified reads (%)')
```
...
#             'supplemental',
#             'read depth',
#             paste0(plotID, '_Unassigned reads by study (histogram).pdf')),
#        height = 4, width = 5)
```

Maybe another vis, to make outliers clearer?
